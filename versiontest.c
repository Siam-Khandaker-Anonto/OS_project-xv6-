...
static void writefile(const char *name, const char *data)
{
  ...
  if(fd < 0){
    printf(1, "ERROR: cannot open %s for writing\n", name);
    return;
  }
...
main(int argc, char *argv[])
{
  ...
  if(count == 3)
    ...
    printf(1, "  (expected 3 snapshots, found %d)\n", count);
  ...
  if(v_one >= 0 && v_two >= 0 && v_three >= 0)
    ...
    printf(1, "  v_one=%d v_two=%d v_three=%d\n", v_one, v_two, v_three);
  ...
  printf(1, "--- listversions output ---\n");
  listversions("a.txt");
  printf(1, "--- end listversions ---\n");
  ...
  if(ret < 0){
    ...
    if(strcmp(buf, saved) == 0)
      ...
      printf(1, "  file was modified after failed restore!\n");
  ...
  printf(1, "\n");
  if(failed == 0)
    printf(1, "ALL TESTS PASSED (%d/%d)\n", passed, passed);
  else
    printf(1, "SOME TESTS FAILED (%d passed, %d failed)\n", passed, failed);
